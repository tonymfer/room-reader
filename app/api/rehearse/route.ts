...
function isValidReport(
  ...
  if (!isObject(scores) || !Array.isArray(reactions) || !isObject(consensus))
    return false;
  ...
  return (
    Array.isArray(consensus.whatLanded) &&
    Array.isArray(consensus.hardestQuestions)
  );
}
...
function normalizePersonaIdentity(
  report: RoomReport,
  personas: Persona[],
): RoomReport {
  return {
...
function slimPersona(p: Persona) {
  return {
    id: p.id,
    name: p.name,
    role: p.role,
...
Registered audience cards JSON:\n${JSON.stringify(personas.map(slimPersona))}
...
export async function POST(request: Request) {
  ...
  try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You return strict JSON only. You are concise, concrete, safe, and demo-oriented.",
          },
          {
            role: "user",
            content: buildPrompt(topic.id, transcript, personas),
          },
        ],
      }),
    ...
    if (!isValidReport(parsed, personas.length))
      throw new Error("OpenAI JSON did not match RoomReport shape");
